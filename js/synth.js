...
// Funktion für Frequenzen: 7.7169265821231ℯ^(0.0577622650467x)
// var anon = function (a, b) { return a + b };
var freqFunc = function (x) { return 7.7169265821231 * Math.exp(0.0577622650467*x) }
...
console.log("velocityVolumes.length: " + velocityVolumes.length)
console.log("velocityVolumes[0].length: " + velocityVolumes[0].length)


// IG: einmalig aufrufen?!
...
function startNote(note, velocity) {
    ...
    for (let i = 0; i < 2; i++) {
        ...
        // for panning
        oscillators[i][note].connect(panNode);
...
function startNoteFreq(freq, velocity) {
    // IG: kopie der Funktion startNote --> statt note wird direkt freq verwendet
    // console.log("startNoteFreq: freq" + String(freq));
    let oscTypes = ["sine", "sawtooth"];

    for (let i = 0; i < 2; i++) {
        oscillators[i][freq] = context.createOscillator();
        oscillators[i][freq].type = oscTypes[i];
        oscillators[i][freq].frequency.value = freq;
        if (i === 1) { oscillators[i][freq].detune.value = 2;}

        velocityVolumes[i][freq].gain.cancelScheduledValues(0);
        velocityVolumes[i][freq].gain.setValueAtTime(0, context.currentTime);

        lfoGain.connect(velocityVolumes[i][freq].gain);
        
        oscillators[i][freq].connect(velocityVolumes[i][freq]);

        // for panning
        oscillators[i][freq].connect(panNode);
        panNode.connect(context.destination);

        oscillators[i][freq].start();

        //attack
        velocityVolumes[i][freq].gain.linearRampToValueAtTime(0.05 + (0.33 * (velocity/127)), context.currentTime + attack);
    }
}

function stopNoteFreq(freq, velocity) {
    // console.log("stopNoteFreq: freq" + String(freq));
    // IG: kopie der Funktion stopNote --> statt note wird direkt freq verwendet
    for (let i = 0; i < 2; i++) {
        //release
        velocityVolumes[i][freq].gain.cancelScheduledValues(0);
        velocityVolumes[i][freq].gain.linearRampToValueAtTime(0, context.currentTime + attack + release);
        oscillators[i][freq].stop(context.currentTime + attack + release + 0.1);
    }
}
